...
def test_input_shape():
    ...
    model, device = image_harmonize.get_harmonize_model()
    ...
    for count in range(N):
        ...
        m = torch.randn(B, 1, H + h, W + w)

        # print("x: ", x.size())

        start_time = time.time()
        with torch.no_grad():
            y = model(x.to(device), m.to(device))
...
def run_bench_mark():
    ...
    with torch.profiler.profile(
        ...
        for ii in range(N):
            ...
            with torch.no_grad():
                y = model(image.to(device), mask.to(device))
            torch.cuda.synchronize()
        p.step()
...
def export_onnx_model():
    ...
    output_names = ["output"]
    onnx_filename = "output/image_harmonize.onnx"
    ...
    onnx_inputs = {input_names[0]: to_numpy(dummy_input), input_names[1]: to_numpy(dummy_mask)}
